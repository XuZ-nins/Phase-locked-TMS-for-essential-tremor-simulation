...
if ispc
	copyfile('nrnmech.dll','irTMS_par_5p/ET_par_hoc');
else
	system('cp -r x86_64 irTMS_par_5p/ET_par_hoc');
end
...
if numCPUs > 1 && par_on
    ...
    parfor mm = 1:size(y_all,1)
		...
		if exist(strcat('irTMS_par_5p/simulation_',num2str(mm),'/recordings_full/Vimap.txt'),'file')
			continue;
		end
    	...
    	mkdir(strcat('irTMS_par_5p/simulation_',num2str(mm),'/recordings_full'));
        IONstim = 2200;
	...
	%     stimlist = stimlist - 2600;
    	...
    	writeVectorBin(strcat('irTMS_par_5p/simulation_',num2str(mm),'/'),tvec,ivec); % write tvec, Evec to params/test

    	system(strcat('cp -r irTMS_par_5p/ET_par_hoc/*',' irTMS_par_5p/simulation_',num2str(mm),'/'));
    	system(strcat('cp initialization/currentState.dat irTMS_par_5p/simulation_',num2str(mm),'/'));

        % Specify paths for storing simulation files
        formatSpec = '%s \n';
    	opt_fileID = fopen(strcat('irTMS_par_5p/simulation_',num2str(mm),'/trialname_par.txt'),'w');
        ...
        formatSpec = '%.8f \n';
    	opt_fileID = fopen(strcat('irTMS_par_5p/simulation_',num2str(mm),'/IONparams.txt'),'w');
        fprintf(opt_fileID,formatSpec,IONstim);
        fclose(opt_fileID);

        formatSpec = '%d \n';
    	opt_fileID = fopen(strcat('irTMS_par_5p/simulation_',num2str(mm),'/prcparams.txt'),'w');
        fprintf(opt_fileID,formatSpec,stimlist);
        fclose(opt_fileID);
        system(strcat('cp irTMS_par_5p/simulation_',num2str(mm),'/prcparams.txt irTMS_par_5p/simulation_',num2str(mm),'/recordings_full/stimlist.txt'))

    	cd(strcat('irTMS_par_5p/simulation_',num2str(mm),'/'));
