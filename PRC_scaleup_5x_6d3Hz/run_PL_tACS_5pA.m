...
if ispc
	copyfile('nrnmech.dll','PL_tACS_par_5pA/ET_par_hoc');
else
	system('cp -r x86_64 PL_tACS_par_5pA/ET_par_hoc');
end
...
ampparam = 0.005; % Stimulation amplitude (in nA)
...
if numCPUs > 1
    ...
    parfor mm = run_range
		if exist(strcat(par_dir,'/PL_tACS_par_5pA/simulation_',num2str(mm)),'dir')
			continue;
		end
		...
		mkdir(strcat(par_dir,'/PL_tACS_par_5pA/simulation_',num2str(mm),'/recordings_full'));
        system(strcat('cp -r PL_tACS_par_5pA/ET_par_hoc/*',' PL_tACS_par_5pA/simulation_',num2str(mm),'/'));
        system(strcat('cp -r initialization/currentState.dat PL_tACS_par_5pA/simulation_',num2str(mm),'/'));
        system(strcat('cp -r initialization/* PL_tACS_par_5pA/simulation_',num2str(mm),'/recordings_full','/'));
		
		cd(strcat(par_dir,'/PL_tACS_par_5pA/simulation_',num2str(mm),'/'));
