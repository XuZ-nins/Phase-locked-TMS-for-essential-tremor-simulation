...
if ispc
	copyfile('nrnmech.dll','PL_TMS_par_15p_s1/ET_par_hoc');
else
	system('cp -r x86_64 PL_TMS_par_15p_s1/ET_par_hoc');
end
...
if numCPUs > 1
    ...
    parfor mm = run_range
		if exist(strcat(par_dir,'/PL_TMS_par_15p_s1/simulation_',num2str(mm)),'dir')
			continue;
		end
		...
		mkdir(strcat(par_dir,'/PL_TMS_par_15p_s1/simulation_',num2str(mm),'/recordings_full'));
        system(strcat('cp -r PL_TMS_par_15p_s1/ET_par_hoc/*',' PL_TMS_par_15p_s1/simulation_',num2str(mm),'/'));
        system(strcat('cp -r initialization/currentState.dat PL_TMS_par_15p_s1/simulation_',num2str(mm),'/'));
        system(strcat('cp -r initialization/* PL_TMS_par_15p_s1/simulation_',num2str(mm),'/recordings_full','/'));
		
		cd(strcat(par_dir,'/PL_TMS_par_15p_s1/simulation_',num2str(mm),'/'));
        ...
        for i = (2000/t_step+1):(t_stop/t_step) % 10 seconds
            ...
            if mod(i,1/t_step)==0
				...
				if (tmpphase>=tmpPRC && tmpphase_pre<=tmpPRC) || ...
					...
					if i>2200/t_step && delaycounter<i-1.5*tremorT/t_step
						if initflg>0
							initflg = initflg-1;
							fprintf('Time: %f\n',i*t_step);
							...
							delparam = i*t_step+1;
							fprintf('Time: %f\n',i*t_step);
							if ~isempty(stimlist) && delparam<stimlist(end)+tremorT*1.5
