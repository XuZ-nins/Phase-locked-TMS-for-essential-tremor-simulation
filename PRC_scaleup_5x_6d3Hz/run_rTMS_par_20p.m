...
if ispc
	copyfile('nrnmech.dll','rTMS_par_20p/ET_par_hoc');
else
	system('cp -r x86_64 rTMS_par_20p/ET_par_hoc');
end
...
y_all = [];
Periodlist = 1:0.1:1.9;
for k = 1:length(Periodlist)
    for l = 1:length(Initlist)
        x = [Periodlist(k);Initlist(l)];
        y = [k,l];
        y_all = [y_all;y];
    end
end
...
if numCPUs > 1 && par_on
    ...
    parfor mm = 1:size(y_all,1)
		...
		if exist(strcat('rTMS_par_20p/simulation_',num2str(mm),'/recordings_full/Vimap.txt'),'file')
			continue;
		end

        disp(mm);
        y = y_all(mm,:);
        tmpPeriod = Periodlist(y(1));
        tmpInit = Initlist(y(2));

    	mkdir(strcat('rTMS_par_20p/simulation_',num2str(mm),'/recordings_full'));
        fprintf('Stimulation period: %f\n',tmpPeriod);
        IONstim = 2200;

        delparam = tmpInit+IONstim+ceil(tremor_T);
        stimlist = delparam:(tremor_T*tmpPeriod):10000;
    	...
    	writeVectorBin(strcat('rTMS_par_20p/simulation_',num2str(mm),'/'),tvec,ivec); % write tvec, Evec to params/test

    	system(strcat('cp -r rTMS_par_20p/ET_par_hoc/*',' rTMS_par_20p/simulation_',num2str(mm),'/'));
    	system(strcat('cp initialization/currentState.dat rTMS_par_20p/simulation_',num2str(mm),'/'));

        % Specify paths for storing simulation files
        formatSpec = '%s \n';
    	opt_fileID = fopen(strcat('rTMS_par_20p/simulation_',num2str(mm),'/trialname_par.txt'),'w');
        ...
        f_20p = strcat('recordings_full/iTMS.txt');
        ...
        fprintf(opt_fileID,formatSpec,f_20p);
        fclose(opt_fileID);

        formatSpec = '%.8f \n';
    	opt_fileID = fopen(strcat('rTMS_par_20p/simulation_',num2str(mm),'/IONparams.txt'),'w');
        fprintf(opt_fileID,formatSpec,IONstim);
        fclose(opt_fileID);

        formatSpec = '%d \n';
    	opt_fileID = fopen(strcat('rTMS_par_20p/simulation_',num2str(mm),'/prcparams.txt'),'w');
        fprintf(opt_fileID,formatSpec,stimlist);
        fclose(opt_fileID);
        system(strcat('cp rTMS_par_20p/simulation_',num2str(mm),'/prcparams.txt rTMS_par_20p/simulation_',num2str(mm),'/recordings_full/stimlist.txt'))

    	cd(strcat('rTMS_par_20p/simulation_',num2str(mm),'/'));
