...
if ispc
	copyfile('nrnmech.dll','rTMS_par_0p/ET_par_hoc');
else
	system('cp -r x86_64 rTMS_par_0p/ET_par_hoc');
end
...
for mm = 1
	disp(mm);
	mkdir(strcat('rTMS_par_0p/simulation_',num2str(mm),'/recordings_full'));
	IONstim = 2200;

	system(strcat('cp -r rTMS_par_0p/ET_par_hoc/*',' rTMS_par_0p/simulation_',num2str(mm),'/'));
	system(strcat('cp initialization/currentState.dat rTMS_par_0p/simulation_',num2str(mm),'/'));

	% Specify paths for storing simulation files
	formatSpec = '%s \n';
	opt_fileID = fopen(strcat('rTMS_par_0p/simulation_',num2str(mm),'/trialname_par.txt'),'w');
	...
	formatSpec = '%.8f \n';
	opt_fileID = fopen(strcat('rTMS_par_0p/simulation_',num2str(mm),'/IONparams.txt'),'w');
	fprintf(opt_fileID,formatSpec,IONstim);
	fclose(opt_fileID);

	cd(strcat('rTMS_par_0p/simulation_',num2str(mm),'/'));
