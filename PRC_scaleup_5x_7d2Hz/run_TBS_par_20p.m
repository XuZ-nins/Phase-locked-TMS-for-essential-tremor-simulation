...
if ispc
	copyfile('nrnmech.dll','TBS_par_20p/ET_par_hoc');
else
	system('cp -r x86_64 TBS_par_20p/ET_par_hoc');
end
...
if numCPUs > 1 && par_on
    ...
    parfor mm = 1:length(Initlist)
		...
		if exist(strcat('TBS_par_20p/simulation_',num2str(mm),'/recordings_full/Vimap.txt'),'file')
			continue;
		end
    	...
    	mkdir(strcat('TBS_par_20p/simulation_',num2str(mm),'/recordings_full'));
    	...
    	writeVectorBin(strcat('TBS_par_20p/simulation_',num2str(mm),'/'),tvec,ivec); % write tvec, Evec to params/test

    	system(strcat('cp -r TBS_par_20p/ET_par_hoc/*',' TBS_par_20p/simulation_',num2str(mm),'/'));
    	system(strcat('cp initialization/currentState.dat TBS_par_20p/simulation_',num2str(mm),'/'));

        % Specify paths for storing simulation files
        formatSpec = '%s \n';
    	opt_fileID = fopen(strcat('TBS_par_20p/simulation_',num2str(mm),'/trialname_par.txt'),'w');
        ...
        formatSpec = '%.8f \n';
    	opt_fileID = fopen(strcat('TBS_par_20p/simulation_',num2str(mm),'/IONparams.txt'),'w');
        fprintf(opt_fileID,formatSpec,IONstim);
        fclose(opt_fileID);

        formatSpec = '%d \n';
    	opt_fileID = fopen(strcat('TBS_par_20p/simulation_',num2str(mm),'/prcparams.txt'),'w');
        fprintf(opt_fileID,formatSpec,stimlist);
        fclose(opt_fileID);
        system(strcat('cp TBS_par_20p/simulation_',num2str(mm),'/prcparams.txt TBS_par_20p/simulation_',num2str(mm),'/recordings_full/stimlist.txt'))

    	cd(strcat('TBS_par_20p/simulation_',num2str(mm),'/'));
